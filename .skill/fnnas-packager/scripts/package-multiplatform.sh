...
# 清理旧的 fpk 产物
echo ""
echo -e "${YELLOW}🧹 清理旧 fpk 文件...${NC}"
rm -f "$RELEASE_DIR/$VERSION"/*.fpk 2>/dev/null || true
rm -f "$PROJECT_ROOT"/*.fpk 2>/dev/null || true
...
for BINARY_FILE in "${BINARY_FILES[@]}"; do
    ...
    if [ -d "$PLATFORM_DIR/img" ]; then
        rm -rf "$OUTPUT_DIR/app/www/img"
        cp -r "$PLATFORM_DIR/img" "$OUTPUT_DIR/app/www/"
    fi
...
if [ -d "$RELEASE_DIR/$VERSION/lottery-assistant-$VERSION-linux-amd64/img" ]; then
    rm -rf "$FUNNAS_DIR/app/www/img"
    cp -r "$RELEASE_DIR/$VERSION/lottery-assistant-$VERSION-linux-amd64/img" "$FUNNAS_DIR/app/www/"
    echo -e "${GREEN}✓ 已同步 img 到 $FUNNAS_DIR/app/www/img/${NC}"
fi
